...
@rx.event
def _on_click_graph(event: Dict[str, Any]) -> list[rx.Var]:
    return [event]
...
@rx.event
def _on_double_click_node(node_id: str, node: Dict[str, Any]) -> list[rx.Var]:
    return [node_id, node]

@rx.event
def _on_right_click_node(event: Dict[str, Any], node_id: str, node: Dict[str, Any]) -> list[rx.Var]:
    return [event, node_id, node]

@rx.event
def _on_mouse_over_node(node_id: str, node: Dict[str, Any]) -> list[rx.Var]:
    return [node_id, node]

@rx.event
def _on_mouse_out_node(node_id: str, node: Dict[str, Any]) -> list[rx.Var]:
    return [node_id, node]

@rx.event
def _on_click_link(source:str, target:str) -> list[rx.Var]:
    return [source, target]

@rx.event
def _on_right_click_link(event: Dict[str, Any], source: str, target: str) -> list[rx.Var]:
    return [event, source, target]

@rx.event
def _on_mouse_over_link(source :str, target :str) -> list[rx.Var]:
    return [source, target]

@rx.event
def _on_mouse_out_link(source :str, target :str) -> list[rx.Var]:
    return [source, target]

@rx.event
def _on_node_position_change(node_id : str, x: float, y : float) -> list[rx.Var]:
    return [node_id, x, y]

@rx.event
def _on_zoom_change(previous_zoom : Dict[str, Any], new_zoom: Dict[str, Any]) -> list[rx.Var]:
